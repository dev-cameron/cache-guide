...
const AnimBox: React.FC<AnimBoxProps> = ({ activeNode }) => {
  ...
  */
  ...
  return (
    <div className="border border-universal-neutral relative flex items-center h-28 bg-universal-neutral-accent rounded-lg my-2 mx-2 drop-shadow-xl">
      <div>
      ...
      {/* "relative flex items-center w-full h-20 text-universal-neutral-contrast" */}
      <div className="relative flex items-center w-full h-20 text-universal-neutral-contrast">
        <div ref={clientRef} className={clsx("flex flex-col items-center justify-center basis-1/5", {
          "text-white": activeNode === "clientRef", 'text-universal-neutral': activeNode !== 'clientRef'
        })}>
        ...
        <div ref={cacheRef} className={clsx("flex flex-col items-center justify-center basis-1/5", {
          "text-white": activeNode === "cacheRef", 'text-universal-neutral': activeNode !== 'cacheRef'
        })}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-12 h-12">
        ...
        <div ref={serverRef} className={clsx("flex flex-col items-center justify-center basis-1/5", {
          "text-white": activeNode === "serverRef", 'text-universal-neutral': activeNode !== 'serverRef'
        })}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-12 h-12">
        ...
        <div className="absolute z-10 -top-3 text-universal-neutral-accent bg-universal-neutral transition-all duration-300 rounded-md p-1 drop-shadow-xl" style={{ left: activeNode==="serverRef" ? `${x-40}px` : `${x+2}px` }}>
...
const InfoBox: React.FC<InfoBoxProps> = ({ activeScenario, activeFrame }) =>{
  return (
    <div className="flex flex-col space-y-4 lg:space-y-0 lg:flex-row lg:space-x-4 w-full rounded-lg py-2 px-2">
      <div className="border border-universal-neutral flex flex-col bg-universal-neutral-accent h-32 lg:h-48 lg:w-1/2 rounded-lg py-2 px-4 drop-shadow-xl">
        <div className="text-white font-bold">
          HTTP Message Type: <span className="px-2 rounded-md bg-universal-neutral text-white shadow-inner">{ activeScenario.frames[activeFrame].type }</span>
        </div>
        <div className="h-full my-1 p-1 rounded-md bg-universal-neutral text-white font-mono text-sm overflow-y-auto">
        { 
...
const ControlBox: React.FC<ControlBoxProps> = ( { updateActiveFrame, frameCount, activeFrame } ) => {
  ...
  for (let i = 0; i < frameCount; i++) {
    frameDots.push(
      // below are 2 ways to conditionally style elements. 1st uses clsx, 2nd is ternary
      // clsx is great you have multiple conditions to check, whereas ternary supports only 2
      <div className={clsx("bg-universal-neutral w-2 h-2 rounded-xl",{
        "bg-white": i === activeFrame
      })}></div>
      // <div className={i === activeFrame ? "bg-universal-neutral w-2 h-2 rounded-xl" : "bg-white w-2 h-2 rounded-xl" }></div>
    )
  }
  
  return (
    <div className="flex items-center justify-left space-x-2 w-full h-10 rounded-lg mx-2">
      <button className="border border-universal-neutral text-white rounded-lg bg-universal-neutral-accent hover:contrast-150 py-1 px-2 drop-shadow-xl"onClick={()=>updateActiveFrame(false)}>prev</button>
      <button className="border border-universal-neutral text-white rounded-lg bg-universal-neutral-accent hover:contrast-150 py-1 px-2 drop-shadow-xl"onClick={()=>updateActiveFrame(true)}>next</button>
